...
\begin{document}
	...
	\begin{wrapfigure}{R}{.35\textwidth}
		...
		\caption{Pathelogical case for ICTS-m}
		\label{fig:path}
	\end{wrapfigure}
	...
	When searching MDDs for a solution in regular MAPF, it is guaranteed that there exists a goal node in the joint MDD without vertex conflicts. This follows from the fact that goals are assumed to be unique per agent. In MAPFM, for a large number of nodes that are searched, this is not the case. As a result, analogously to how pathfinding algorithms will more-or-less floodfill a graph if there exists no path to the goal, the entire joint MDD space will have to be searched which becomes very expensive as the number of agents increases.
	
	Two strategies could be used to avoid this: firstly, the MDD search could be changed to start at the maximal depth and search for a way towards the start (which is still guaranteed to be a nonconflicting node); secondly, a bipartite matching algorithm could be employed to see if any matching exists between agents and goals they can reach. An advantage of the former strategy is that it will, in particular with larger team sizes, reduce the number of maximal depth nodes drastically, so that the subsequent search is from few nodes to one goal. By only running a matching algorithm to skip regular MDD search, there is no such reduction so from one starting position paths will be searched to potentially many goal nodes, of which many will end up being illegal. 
	
	[TODO: either describe inverted MDD search or the use of some bipartite matching algorithm that shortcuts the regular MDD search]
	...
	\begin{subfigure}{0.49\textwidth}
		...
		\label{fig:75-3-p}
	\end{subfigure}
	\caption{Fraction solved out of 50 maps of algorithm variants on two types of 20x20 grids with a 120s timeout}
	\label{fig:probs}
	\end{figure*}
	\begin{figure*}
		\centering
		...
		\begin{subfigure}{0.49\textwidth}
			...
			\label{fig:75-3}
		\end{subfigure}
		\caption{Average solving time of algorithm variants on two types of 20x20 grids with a 120s timeout}
		\label{fig:times}
	\end{figure*}
	...
	Across exhaustive variants, with exception of the ID variant, a type of hierarchy can be distinguished, as best illustrated by \ref{fig:25-3-p}. Almost without exception, Exh+E is outperformed by Exh+E+B which similarly is outperformed by Exh+E+B+O which is again outperformed by Exh+S+C+B+O. Exh+E being outperformed by Exh+E+B was to be expected, given that Exh+E+B does at most as much work as Exh+E while in practice often being able to search ICT's much less deep as more instances are solved. Exh+E+B+O outperforming Exh+E+B is not quite as obvious from a theoretical standpoint, as discussed in \ref{ordered-enum}. Although no bound-value data was recorded, one hypothetical explanation might be that often, early on in the search, single matchings with low $\Delta_m$ are found, causing the rest of the searches to be significantly curtailed.
	
	
	Exh+S+C+B+O outperforming Exh+E+B+O shows the potential of the child pruning method described relative to the enhanced pruning method. In particular as $k$ grows larger and in scenarios with more obstacle (\ref{fig:75-1-p},\ref{fig:75-3-p}), it appears that having to search a smaller ICT is more important in terms of performance than being able to skip the $k$-agent low-level search more often.
	
	Simple pruning with child pruning outperforming the enhanced pruning variant.
	
	
	 
	With some exceptions, the ICTS-m 
