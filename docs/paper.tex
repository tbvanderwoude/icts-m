...
\begin{document}
	...
	If all teams are singleton, the MAPFM instance is equivalent to a MAPF instance; if there is just one team, this corresponds to an anonymous MAPF problem that can be solved in polynomial time.
	...
	As a further improvement of this method, the matchings $m$ could be enumerated and solved in increasing order of $C^*_m$. In this case, the effort per matching is monotonically non-increasing by Property \ref{monotonicity}. As will be shown in section \ref{experiments}, this monotonic reduction of effort appears to be a desirable property in practice but pathelogical cases could be constructed where the $\Delta_{m,bounded}$ of $m$ with low $C^*_m$ is much higher, thus dominating the performance.
	...
	\end{proof}
	\begin{corollary}[Reduced upper bound]
	...
	Two different experiments were run: one comparing ICTS-based algorithms discussed in this work, and one comparing one of these, the approximate ordered enumeration variant (Exh+E+B+O in the relative comparisons), to other algorithms for MAPFM developed within the peer group\footnote{I want to thank Jonathan Dönszelmann, who worked on M*-based algorithms, for setting up this comparison[TODO: move to thanks note at end]}.
	
	In experiments used to compare the different MAPFM algorithms within the peer group, care was taken to make the comparison fair: all algorithms are implemented in Python 3 and run on the same system: a server with a 12-core Intel Xeon E5-2683 with 8Gb of RAM dedicated to running experiments for this project.
	
	The results are shown in Figure \ref{fig:times} and \ref{fig:probs}, which respectively show the average time to solution and the fraction of problems solved out of 50 instances. The algorithms and interpretation of results are discussed below.
