...
\begin{document}
	...
	\end{property}
	\begin{proof}
		Due to the order of matchings for successive matchings $m,m+t$ for some $t\geq 1$, $C^*_{m} \leq C^*_{m+t}$. By definition, $B_{m+1} = \min(B_m,C_m)$ meaning that $B_{m+1} \leq C_m$ and $B_{m+1} \leq B_m$ so by induction $B_{m+t} \leq C_m$ and similarly $B_{m+t} \leq B_m$. Therefore 
		\[C^*_{m+t} - B_{m+t} \leq C^*_m - B_m \implies \Delta_{m+t,bounded} \leq \Delta_{m,bounded}\]
	\end{proof}
	In practice, it is quite difficult enumerate all matchings in increasing order of root SIC. Finding a lowest SIC $m$ reduces to the assignment problem that can be solved using the Hungarian algorithm \cite{kuhn55} in polynomial time; however, to the knowledge of the author, no algorithm exists to more generally compute the $i$'th matching in increasing order of $C^*_m$ (assuming some fixed tie-breaking for equivalent matchings). This means that matchings have to be exhaustively enumerated and stored in memory to sort them, but this requires $\mathcal{O}(M)$ memory which was found to be more problematic in implementation than the corresponding time complexity of the sorting, $\mathcal{O}(M\log(M))$.
	
	To still make use of a form of monotonicity, a
	\subsection{ICTS-m}
	...
	\end{property}
	\begin{proof}
		Assume there would be a solution with $C_k < C_i + C_j$. Taking $C_k^i$ to denote the SIC of agents that used to make up $g_i$ in the solution to $g_k$, without loss of generality $C_k^i < C_i$, which contradicts the optimality of $C_i$ as solution for $g_i$ independent of $g_j$.
	\end{proof}
	...
	An optimization of ICTS that hitherto appears to not have been described in the literature was found in the process of optimizing ICTS-m. The idea is simple: when pruning, in the original ICTS formulation all $k$-children are generated in every case because pruning only controls skipping the low-level search, assumed to be the most expensive operation. However, in simple pruning in any case, when there is a conflict between two or more agents at a given ICT node $(c_1,\ldots,c_k)$, in any solution, one or more of the path costs corresponding to these conflicting agents will have to be incremented. This reduces the number of children generated to the number of conflicting agents, which quite drastically can improve the branching factor of the ICT search if say $k = 12$ but a conflicting pair is found.
	
	Child pruning cannot be combined with enhanced pruning whilst retaining optimality due to the \textit{cascading effect} that Sharon et al. describe in the original ICTS paper\cite{sharon2011}. Consider Figure \ref{fig:counter-example-ep}. It can be seen that $(3,8,5)$ with SIC $16$ is the optimal solution here, which corresponds to $red$ moving aside, allowing $green$ to pass.  Say enhanced pruning with pairs is applied to these three agents at node $(c_{red},c_{green},c_{gray}) = (0,8,5)$.
	Let the enhanced pruning order be $(red,green)$, $(red,gray)$, $(green,gray)$. $(red,green)$ is solvable but will leave for $green$ out of the two paths of length $8$ only the clock-wise path. $(red,gray)$ will not change either MDD. When $(green,gray)$ is evaluated, it is found that the remaining clockwise path for $green$ and $gray$'s path are incompatible (would require swapping) so $(0,9,5)$ and $(0,8,6)$ are generated. Proceeding similarly for these two nodes, for neither a solution is found so from $(0,9,5)$, $(0,10,5),(0,9,6)$ and from $(0,8,6)$, $(0,9,6),(0,8,7)$. In the end, with this pruning scheme, nodes
	...
	\end{figure*}
	In enhanced pruning, it is less clear if in every case it is safe to only generate nodes corresponding to conflicting agents. This is due to the \textit{cascading effect} that Sharon et al. describe in the original ICTS paper\cite{sharon2011}. Consider ICT node $(c_1,c_2,c_3)$ with MDDs $MDD_1^{c_1},MDD_2^{c_2},MDD_3^{c_3}$
	
	 in enhanced pruning with pairs, even if $MDD_i,MDD_j$ have a solution, as part of the search routine the MDDs could have become sparser due to totally incompatible children being pruned in the process. If now $MDD_i$ and some $MDD_k$ are searched and no solution is found, in child pruning only children for these two nodes would be generated while in the optimal solution, $MDD_i$ might need its $c_i$ increased.
	
	\begin{conjecture}
		Enhanced pruning combined with child pruning is suboptimal.
	\end{conjecture}
