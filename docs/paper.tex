...
\begin{document}
	...
	Such methods are suboptimal so given a TUSS instance, a found heuristic solution gives a (loose) upper bound for the optimal solution cost; solving an instance also shows its feasibility, although there might be feasible instances for which no heuristic solution is found.
	By defining instances for a set shunting yard, heuristic methods can be used to characterise lower-bounds on the capacity of the yard in terms of both instance feasibility and solution cost.
	Because the quality of found heuristic solutions cannot be estimated \cite{mulderij2020}, it cannot be known how close such lower-bounds for yard capacity are to the true capacity. 
	Combined with the fact that current upper-bounds for capacity are highly optimistic, this makes it difficult for rail companies to decide when to expand their shunting infrastructure.
	
	Mulderij et al. propose the multi-agent pathfinding (MAPF) problem, as described in \cite{stern2019}, extended with a matching subproblem (hereafter MAPFM) as a suitable relaxation of (discretised) TUSS for finding better upper-bounds using an approach analogous to that outlined above \cite{mulderij2020}.
	...
	Comparatively little research has been done on MAPFM and solving it optimally. For MAPFM with a makespan objective instead of SIC, known as the combined target-assignment and pathfinding (TAPF) problem, there is the Conflict-Based Min-Cost-Flow (CBM) algorithm by Ma and Koenig \cite{ma2016} and a similar approach described in \cite{henkel2019}. Ma and Koenig's method uses the Meta-Agent variant of CBS \cite{sharon2015} as a high-level search framework; in the low-level search, single teams are planned in polynomial time as constrained anonymous MAPF instances using a max-flow approach \cite{yu2013}. 
	
	The experiments reported in \cite{ma2016} indicate that this method scales well as the number of agents and the sizes of teams increase; although in addition to grid instances with 10\% of tiles being blocked, Kiva \cite{wurman2007} instances are also solved, it remains to be seen how CBM performs in scenarios with more obstacles and more maze-like scenarios, such as those included in the Moving AI benchmarks \cite{sturtevant2012} for MAPF.
	...
	\end{itemize}
	In this work, following the definition of the TUSS relaxation in \cite{mulderij2020}, an optimal solution is defined as having minimal SIC.
