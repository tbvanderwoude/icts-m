...
class SolveTestTwo(unittest.TestCase):
    ...
    def test_solve_one(self):
        problem: Problem = self.instance_creator(MarkedLocation(0, 1, 1),MarkedLocation(0, 1, 2),MarkedLocation(0,2,1),MarkedLocation(0,2,2))
        solution = solve(problem)
        routes = list(map(lambda pi: pi.route,solution.paths))
        self.assertListEqual(routes, [[(1,1),(2,1)],[(1,2),(2,2)]])

    def test_solve_many(self):
        problem: Problem = self.instance_creator(MarkedLocation(0, 1, 1),MarkedLocation(1, 1, 2),MarkedLocation(0,2,2),MarkedLocation(1,2,1))
        solution = solve(problem)
        routes = list(map(lambda pi: pi.route,solution.paths))
        self.assertListEqual(routes, [[(1,1),(2,1),(2,2)],[(1,2),(1,1),(2,1)]])

    def test_solve_swap(self):
        problem: Problem = self.instance_creator(MarkedLocation(0, 1, 1),MarkedLocation(0, 2, 2),MarkedLocation(0, 2, 2),MarkedLocation(0,1,1))
        solution = solve(problem)
