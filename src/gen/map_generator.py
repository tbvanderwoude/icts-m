...
class MapGenerator:
    ...
    def generate_map(
        ...
        while True:
            grid = self.__generate_maze(
                width, height, open_factor=open_factor, max_neighbors=max_neighbors
            )
            count_traversable = 0
            ...
            if (
                count_traversable
                < (open_factor * width * height * 0.25 * max_neighbors)
                ...
                while result is None:
                    try:
                        # connect
                        result = self.__generate_agent_positions(
                            grid,
                            width,
                            height,
                            num_agents,
                ...
                for color, team_size in enumerate(num_agents):
                    for _ in range(team_size):
                        start_locations.append(
                            MarkedLocation(color, starts[i].x, starts[i].y)
                        )
                        goal_locations.append(
                            MarkedLocation(color, goals[i].x, goals[i].y)
                        )
                        i += 1
                return Problem(
                    width=width,
    ...
    def generate_even_batch(
        ...
        package_name = (
            ...
            else f"{prefix}-{width}x{height}-A{agents}_T{teams}"
        )
    ...
    def __generate_agent_positions(
        self,
        grid,
        width,
        height,
        ...
        for x in range(sum(num_agents)):
            ...
            while (
                ...
                or Coord(start_x, start_y) in agent_positions
            ):
                start_x = randint(0, width - 1)
                start_y = randint(0, height - 1)
            ...
            for x in range(width):
                ...
                for ndx, ndy in [(0, -1), (0, 1), (-1, 0), (1, 0)]:
                    ...
                    if (
                        ...
                        and grid[neighbor_y][neighbor_x] == 0
                    ):
                        count += 1
                if count == 3:
                    res += 1
    ...
    def __compute_heuristic(
        self, queue: Queue, grid: List[List[int]]
    ) -> Tuple[List[List[Number]], int]:
        visited = set()
        heuristic = [
    ...
    def __get_neighbors(grid: List[List[int]], coords: Coord):
        ...
        for dx, dy in [(0, -1), (0, 1), (-1, 0), (1, 0)]:
            ...
            if (
                0 <= new_coord.y < len(grid)
                and 0 <= new_coord.x < len(grid[0])
                and grid[new_coord.y][new_coord.x] == 0
            ):
    ...
    @staticmethod
    def __generate_maze(
        width: int, height: int, open_factor: float, max_neighbors: int
    ) -> List[List[int]]:
        grid: List[List[int]] = []
